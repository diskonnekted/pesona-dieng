...
<head>
    ...
    <title>Detail Paket: {{ $package['name'] }} | Pesona Dieng</title>
    
    <!-- Meta SEO -->
    <meta name="description" content="Detail lengkap paket wisata {{ $package['name'] }}. Dapatkan detail rencana perjalanan (itinerary), harga terbaik, fasilitas, dan kemudahan booking online.">
    <meta name="keywords" content="paket wisata dieng, tour dieng, {{ $package['name'] }}, itinerary dieng, wisata banjarnegara">
...
<body class="font-inter bg-[#FAF9F6] text-gray-800 antialiased overflow-x-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center gap-4">
                    <a href="{{ route('home') }}#paket" class="text-white hover:text-dieng-gold transition-colors flex items-center gap-2 text-sm font-medium">
    ...
    <section class="relative pt-32 pb-20 bg-dieng-green-dark overflow-hidden flex items-center min-h-[40vh]">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 w-full">
            ...
            <div class="flex items-center gap-2 text-white/60 text-xs md:text-sm mb-4">
                <a href="{{ route('home') }}" class="hover:text-dieng-gold transition-colors">Home</a>
                <i class="fa-solid fa-chevron-right text-[10px]"></i>
                <a href="{{ route('home') }}#paket" class="hover:text-dieng-gold transition-colors">Paket Wisata</a>
                <i class="fa-solid fa-chevron-right text-[10px]"></i>
            ...
            <span class="inline-block bg-dieng-gold/20 border border-dieng-gold/30 text-dieng-gold text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wider mb-3">
                {{ strtoupper($package['category']) }} TOUR
            </span>
            ...
            <div class="flex items-center gap-6 mt-4 text-white/80 text-sm">
                <span class="flex items-center gap-2"><i class="fa-solid fa-star text-yellow-400"></i> <b>{{ $package['rating'] }}</b> ({{ $package['reviews_count'] }} Ulasan)</span>
                ...
                <div class="lg:col-span-2 space-y-10">
                        ...
                        <div class="grid grid-cols-3 gap-3 mt-6">
                            @foreach($package['gallery'] as $img)
                                <div class="overflow-hidden rounded-xl aspect-[4/3] bg-gray-100">
                                    <img src="{{ asset($img) }}" alt="Destinasi Wisata" class="w-full h-full object-cover hover:scale-105 transition-transform duration-300">
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100/80">
                        ...
                        <div class="flex flex-wrap gap-2.5 mb-8 border-b border-gray-100 pb-5">
                            @foreach($package['durations'] as $index => $dur)
                                <button 
                                    onclick="switchDuration('{{ $dur['id'] }}')" 
                                    id="tab-btn-{{ $dur['id'] }}"
                                    class="duration-tab-btn py-3 px-6 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border {{ $index === 0 ? 'bg-dieng-green text-white border-dieng-green shadow-md shadow-dieng-green/10' : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100' }}">
                                    <i class="fa-solid fa-clock text-xs"></i> {{ $dur['label'] }}
                                </button>
                            @endforeach
                        </div>
                        ...
                        @foreach($package['durations'] as $index => $dur)
                            <div id="duration-content-{{ $dur['id'] }}" class="duration-panel space-y-8 {{ $index === 0 ? 'block' : 'hidden' }}">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-8 relative before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-200">
                                        @foreach($dur['itinerary'] as $dayData)
                                            <div class="relative pl-9">
                                                ...
                                                <div class="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                                                    <h4 class="font-bold text-gray-900 text-base md:text-lg mb-4">Hari {{ $dayData['day'] }}: {{ $dayData['title'] }}</h4>
                    ...
                    <div class="bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-gray-100/90 relative overflow-hidden">
                        ...
                        <div class="py-4 border-y border-gray-100 my-5">
                            ...
                            <ul id="features-list" class="space-y-2">
                                @foreach($package['durations'][0]['features'] as $feat)
                                    <li class="flex items-center gap-2 text-xs text-gray-600 font-light">
                                        <i class="fa-solid fa-circle-check text-dieng-gold"></i> <span>{{ $feat }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-16 border-b border-white/10">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex flex-col gap-2.5 text-xs text-white/70 font-light">
                        <a href="{{ route('home') }}#beranda" class="hover:text-dieng-gold transition-colors">Beranda</a>
                        <a href="{{ route('home') }}#destinasi" class="hover:text-dieng-gold transition-colors">Destinasi</a>
                        <a href="{{ route('home') }}#paket" class="hover:text-dieng-gold transition-colors">Paket Wisata</a>
                        <a href="{{ route('home') }}#galeri" class="hover:text-dieng-gold transition-colors">Galeri</a>
                        <a href="{{ route('home') }}#tentang" class="hover:text-dieng-gold transition-colors">Tentang Kami</a>
                    </div>
                </div>
    ...
    <script>
        ...
        const suffix = "{{ $package['slug'] === 'family-fun-dieng' ? 'keluarga' : ($package['slug'] === 'romantic-dieng-honeymoon' ? 'pasangan' : 'pax') }}";
        let selectedDurationId = "{{ $package['durations'][0]['id'] }}";
        let paxCount = 1;
